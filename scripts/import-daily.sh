...
# md はフォルダの中にあるので深さを見る。見つからないときは確認自体を諦めない
sample="$(find "$VAULT" -maxdepth 3 -name '*.md' -print 2>/dev/null | head -1)"
if [ -z "$sample" ]; then
  abort "取り込み元に md が1つも見当たりません（フォルダを読めていない可能性）: $VAULT"
fi
if ! head -c 1 "$sample" >/dev/null 2>&1; then
